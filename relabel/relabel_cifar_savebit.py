...
def quantilization(x):
    return torch.quantize_per_tensor(x,0.005,0,dtype=torch.quint8)
# Train
def train(epoch,wandb_metrics):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        soft_label = soft_label.to(torch.float16)
        soft_label = soft_label.to(torch.float32)
        # soft_label = quantilization(soft_label).dequantize()
