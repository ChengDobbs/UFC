...
model_paths = ["/home/dujw/Robustness/otho_SRE/codes/save/cifar100/resnet18_E200/ckpt.pth","/home/dujw/Robustness/otho_SRE/codes/save/cifar100/mobilenetV2_E200/ckpt.pth","/home/dujw/Robustness/otho_SRE/codes/save/cifar100/efficientnet_E200/ckpt.pth"]
...
model_lists = []
model_types = [torchvision.models.resnet18,MobileNetV2,EfficientNetB0_cifar100]
...
def train(epoch,wandb_metrics):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        for ii in range(len(model_types)):
            ...
            soft_label = model_teacher(original_inputs).detach()
            soft_label = lam* soft_label + (1-lam)*soft_label[mix_index]
            soft_label_avg.append(soft_label.clone().detach())
        soft_label = (soft_label_avg[0] +soft_label_avg[1]+soft_label_avg[2])/3

        dif_1 = torch.abs((soft_label_avg[0] - soft_label)).mean()

        target_integrate = lam*torch.nn.functional.one_hot(target_a,100) + (1-lam)*torch.nn.functional.one_hot(target_b,100)
        dif_2 = torch.abs((target_integrate - soft_label)).mean() 

        calculated_a = model_lists[0](original_inputs).detach()
        calculated_integrate = lam*calculated_a + (1-lam)*calculated_a[mix_index]
        dif_3 = torch.abs((calculated_integrate - soft_label)).mean() 
