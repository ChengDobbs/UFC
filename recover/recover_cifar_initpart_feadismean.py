...
def get_images(args, model_teacher, hook_for_display, ipc_id, feature_previous):
    ...
    for kk in range(0, 100, batch_size):
        ...
        for iteration in range(iterations_per_layer):
            ...
            if ipc_id == 0:
                ...
                feature_previous_flat = feature_previous.view(100, -1)
                feature_flat = feature.view(100, -1)
                cos = nn.CosineSimilarity(dim=1, eps=1e-6)
                cosine_similarity = cos(feature_previous_flat, feature_flat)
                loss_feature = args.feature_scale * cosine_similarity.sum()
    ...
    feature_previous = (feature_previous * ipc_id + feature.detach())/(ipc_id+1)
