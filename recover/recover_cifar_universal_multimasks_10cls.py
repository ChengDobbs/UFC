...
from utils import BNFeatureHook, lr_cosine_policy,  clip_image, denormalize_image
# import wandb
...
def get_images(args, model_lists, hook_for_display, ipc_id):
    ...
    for ii in range(len(model_lists)):
        ...
        for module in model_lists[ii].modules():
            if isinstance(module, nn.BatchNorm2d):
                loss_r_feature_layers.append(BNFeatureHook(module))
        loss_packed_features.append(loss_r_feature_layers)
    ...
    for kk in range(0, 100, batch_size):
        ...
        #calculate which model 
        model_index = ipc_id // (args.ipc_end - 2) - 1
        ...
        # partly reinit_zx
        loaded_tensor = torch.load(args.init_path+'/tensor_'+str(ipc_id%(args.ipc_end - 2))+'.pt').clone().requires_grad_(True).to('cuda')
        input_original = loaded_tensor.detach().clone()[kk : min(kk + batch_size, 100)]
        ...
        uni_perb = torch.zeros((1, 3, 32, 32), requires_grad=True, device="cuda", dtype=data_type)
        uni_perb = uni_perb.to('cuda')
        # partly reinit_zx
        iterations_per_layer = args.iteration
        # ipc - 1 init plus 1 universal perb
        if ipc_id < (args.ipc_end - 2):
...
def main_syn(args, ipc_id):
    ...
    import torchvision
    model_lists = []
    # model_types = [torchvision.models.resnet18,torchvision.models.resnet34,torchvision.models.resnet50]
    # model_types = [EfficientNetB0_cifar100]
    # model_types = [torchvision.models.resnet18,MobileNetV2,ShuffleNetG2_cifar100]
    ...
    # model_paths = ["/home/dujw/Robustness/otho_SRE/codes/save/cifar100/efficientnet_E200/ckpt.pth"]
...
def parse_args():
    ...
    ## modifications_zx
    parser.add_argument('--init_path', default='', type=str)
    parser.add_argument('--init_part_num', default=25, type=int)
...
if __name__ == "__main__":
    ...
    # cause we need n + n synthesize (ipc-2)*4
    for ipc_id in range(args.ipc_start, args.ipc_end * 5 - 10):
        print("ipc = ", ipc_id)
