...
def get_images(args, model_teacher, hook_for_display, ipc_id):
    ...
    for kk in range(0, 100, batch_size):
        ...
        # partly reinit_zx
        # reverse
        # loaded_tensor = torch.load(args.init_path+'/tensor_'+str(49-ipc_id)+'.pt').clone().requires_grad_(True).to('cuda')
        # same init and pruning
...
def parse_args():
    ...
    parser.add_argument("--arch-path", type=str, default="save/cifar100/resnet18_E200/ckpt.pth ")
    ...
    parser.add_argument('--init_part_num', default=1, type=int)
