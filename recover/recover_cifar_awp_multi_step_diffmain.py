...
def get_images(args, model_teacher, hook_for_display, ipc_id):
    ...
    for kk in range(0, 100, batch_size):
        ...
        # partly reinit_zx
        loaded_tensor = torch.load(args.init_path+'/tensor_'+str(ipc_id)+'.pt').clone().requires_grad_(True).to('cuda')
        ...
        for iteration in range(iterations_per_layer):
            ...
            optimizer.zero_grad()
            if iteration ==0:
                for ii in range(args.sam_steps):
            ...
            loss.backward()
            optimizer.step()
            # sam_optimizer.second_step(True)
...
def main_syn(args, ipc_id):
    ...
    if args.networks == 'resnet18':
        ...
        checkpoint = torch.load('/home/dujw/Robustness/otho_SRE/codes/save/cifar100/resnet18_E200/ckpt.pth')
    
    elif args.networks == 'resnet34':
        model_teacher = torchvision.models.resnet34(num_classes=100)
        model_teacher.conv1 = nn.Conv2d(3, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
        model_teacher.maxpool = nn.Identity()
        model_teacher = nn.DataParallel(model_teacher).cuda()
        checkpoint = torch.load('/home/dujw/Robustness/otho_SRE/codes/save/cifar100/resnet34_E200_1/ckpt.pth')

    elif args.networks == 'resnet50':
        model_teacher = torchvision.models.resnet50(num_classes=100)
        model_teacher.conv1 = nn.Conv2d(3, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
        model_teacher.maxpool = nn.Identity()
        model_teacher = nn.DataParallel(model_teacher).cuda()
        checkpoint = torch.load('/home/dujw/Robustness/otho_SRE/codes/save/cifar100/resnet50_E200_1/ckpt.pth')

    elif args.networks == 'resnet101':
        model_teacher = torchvision.models.resnet101(num_classes=100)
        model_teacher.conv1 = nn.Conv2d(3, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
        model_teacher.maxpool = nn.Identity()
        model_teacher = nn.DataParallel(model_teacher).cuda()
        checkpoint = torch.load('/home/dujw/Robustness/otho_SRE/codes/save/cifar100/resnet101_E200/ckpt.pth')
...
def parse_args():
    ...
    parser.add_argument('--sam_steps', default=1, type=int)
