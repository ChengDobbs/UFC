...
# import wandb
from model_feature import resnet18_cifar
...
def get_images(args, model_teacher, hook_for_display, ipc_id, feature_previous):
    ...
    for kk in range(0, 100, batch_size):
        ...
        # partly reinit_zx
        loaded_tensor = torch.load(args.init_path+'/tensor_'+str(ipc_id)+'.pt').clone().requires_grad_(True).cuda()
        inputs = loaded_tensor.detach().clone()
        inputs.requires_grad_(True)
        inputs = inputs.cuda()
        # partly reinit_zx
        iterations_per_layer = args.iteration
        if ipc_id < args.init_part_num:
        ...
        for iteration in range(iterations_per_layer):
            ...
            # feature loss distance mean 
            if ipc_id == 0:
                loss_feature = 0
            else:
                cosine_distances = 0
                for dd in range(0, feature_previous.size(0), 100):
                    feature_previous_flat = feature_previous[dd:dd+100].view(100, -1)
                    feature_flat = feature.view(100, -1)
                    cos = nn.CosineSimilarity(dim=1, eps=1e-6).cuda()
                    cosine_distances = + cos(feature_previous_flat, feature_flat).sum()
                loss_feature = args.feature_scale * cosine_distances/(dd/100+1)


            
            loss = loss_ce + loss_aux + loss_feature
    ...
    if ipc_id == 0:
        feature_previous = feature.detach().clone()
    else:
        feature_previous = torch.cat((feature_previous, feature.detach().clone()), dim=0)

    return feature_previous

def main_syn(args, ipc_id, feature_previous):
    ...
    model_teacher = resnet18_cifar(num_classes=100, pretrained=True, path = args.arch_path).cuda()
    ...
    feature_previous = get_images(args, model_teacher, hook_for_display, ipc_id, feature_previous)
    return feature_previous
...
def parse_args():
    ...
    parser.add_argument("--iteration", type=int, default=10, help="num of iterations to optimize the synthetic data")
    ...
    parser.add_argument("--arch-path", type=str, default="/home/dujw/Robustness/otho_SRE/codes/save/cifar100/resnet18_E200/ckpt.pth")
    ...
    parser.add_argument('--feature_scale', default=0.001, type=float)
...
if __name__ == "__main__":
    ...
    for ipc_id in range(args.ipc_start, args.ipc_end):
        ...
        feature_previous = main_syn(args, ipc_id, feature_previous)
