...
from models.efficientnet import EfficientNetB0_cifar100
from models.shufflenet import ShuffleNetG2_cifar100
from models.vgg_cifar10 import VGG_cifar100
...
def get_images(args, model_teacher_1, model_teacher_2, model_teacher_3, hook_for_display, ipc_id):
    ...
    for kk in range(0, 100, batch_size):
        ...
        for iteration in range(iterations_per_layer):
            ...
            loss_r_bn_feature = [
                mod.r_feature.to(loss_ce_1.device) * rescale[idx] for (idx, mod) in enumerate(loss_r_feature_layers)
            ]
            loss_r_bn_feature = torch.stack(loss_r_bn_feature).sum()

            loss_aux = args.r_bn * loss_r_bn_feature

            loss = 1/3*(loss_ce_1+ loss_ce_2 + loss_ce_3) + loss_aux
...
def main_syn(args, ipc_id):
    ...
    model_teacher_2 = MobileNetV2(num_classes=100)
    ...
    model_teacher_3 = EfficientNetB0_cifar100()
